...
public:
	...
	pqxx::result find_client(std::string search_str) {// найти клиента 
			pqxx::work tx(*conn);
			pqxx::result res = tx.exec("SELECT f_name, s_name, email, phone FROM users JOIN phone on id = id_user WHERE f_name ILIKE '%" + search_str + "%' OR s_name ILIKE '%" + search_str + "%' OR email ILIKE '%" + search_str + "%' OR phone ILIKE '%" + search_str + "%'");
			tx.commit();
			return res;
	}
};
...
int main() {
	...
	try {
		...
		for (auto const& row : db.find_client("322")) { // поиск клиента, функция возвращает контейнер со всей информацией о пользователе
			std::cout << " FOUND CLIENT f_name: " << row["f_name"].as<std::string>()
				<< ", s_name: " << row["s_name"].as<std::string>()
